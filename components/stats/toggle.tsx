...
import { Copy, ExpandingArrow, Tick } from "@/lib/icons";
...
import { Calendar, ChevronDown, Globe, Share2 } from "lucide-react";
...
export default function Toggle() {
  ...
  const isPublicStatsPage = basePath.startsWith("/site");

  return (
    <div
      className={cn(
        "sticky top-[6.85rem] z-10 mb-5 bg-white/10 py-3 backdrop-blur dark:bg-black/10 md:py-5",
        {
          "top-0": isPublicStatsPage,
          "top-6 md:top-0": modal,
          // "shadow-md": scrolled && !modal,
        },
      )}
    >
      <div className="flex h-20 flex-col items-center justify-between space-y-3 px-2.5 md:h-10 md:flex-row md:space-y-0 lg:px-0">
        {slug ? (
          <div className="flex items-center space-x-2">
            <BlurImage
              alt={name || "Project Logo"}
              src={logo || VTAE_LOGO}
              className="h-6 w-6 flex-shrink-0 overflow-hidden rounded-full object-contain"
              width={48}
              height={48}
            />
...
const CustomDomainPopover = () => {
  const { domain } = useContext(StatsContext);
  const [openDomainPopover, setOpenDomainPopover] = useState(false);
  const { queryParams } = useRouterStuff();

  const { customDomain, subdomain, error } = useSite();
  const domains = [
    ...(customDomain ? [customDomain] : []),
    SUB_DOMAIN(subdomain as string),
  ];
  ...
  return (
    <Popover
      ...
      openPopover={openDomainPopover}
      setOpenPopover={setOpenDomainPopover}
    >
      <button
      ...
      >
        ...
        <ChevronDown
          className={`h-5 w-5 text-gray-400 ${
            openDomainPopover ? "rotate-180 transform" : ""
          } transition-all duration-75`}
        />
      </button>
    </Popover>
