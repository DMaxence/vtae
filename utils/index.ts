...
export function constructMetadata({
  ...
  image = "https://vtae.xyz/_static/thumbnail.png",
  icons = [
    {
...
export function linkConstructor({
  ...
  const link = `${
    localhost ? "http://home.localhost:3000" : `https://${domain}`
  }${key && key !== "_root" ? `/${key}` : ""}`;
...
export const isVtaeDomain = (domain: string) => {
  return VTAE_DOMAINS.some((d) => d.slug === domain);
};
...
const logTypeToEnv = {
  cron: process.env.VTAE_SLACK_HOOK_CRON,
  links: process.env.VTAE_SLACK_HOOK_LINKS,
};
...
export const log = async ({
  ...
  if (
    ...
    !process.env.VTAE_SLACK_HOOK_CRON ||
    !process.env.VTAE_SLACK_HOOK_LINKS
  )
    console.log(message);
  ...
  try {
    ...
    console.log(`Failed to log to Vtae Slack. Error: ${e}`);
  }
};
