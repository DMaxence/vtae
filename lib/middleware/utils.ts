...
export const parse = (req: NextRequest) => {
  ...
  if (domain === "vtae.localhost:3000" || domain.endsWith(".vercel.app")) {
    // for local development and preview URLs
    domain = "vtae.xyz";
  }

  // path is the path of the URL (e.g. vtae.co/stats/github -> /stats/github)
  ...
  const key = decodeURIComponent(path.split("/")[1]); // key is the first part of the path (e.g. vtae.co/stats/github -> stats)
  const fullKey = decodeURIComponent(path.slice(1)); // fullKey is the full path without the first slash (to account for multi-level subpaths, e.g. vtae.sh/github/repo -> github/repo)

  return { domain, path, fullPath, key, fullKey };
};

export const getFinalUrl = (target: string, { req }: { req: NextRequest }) => {
  // query is the query string (e.g. vtae.sh/github?utm_source=twitter -> ?utm_source=twitter)
...
export function createMiddleware(middleware: MiddlewareFunction) {
  return (req: NextRequest, res: NextFetchEvent) =>
