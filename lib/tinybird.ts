...
export async function recordVisit({
  ...
  // deduplicate visits from the same IP, domain and key – only record 1 visits per hour
  const { success } = await ratelimit(2, "1 h").limit(
    `recordVisit:${ip}:${domain.toLowerCase()}:${
  ...
  return await Promise.allSettled([
    fetch("https://api.tinybird.co/v0/events?name=click_events&wait=true", {
      method: "POST",
      headers: {
